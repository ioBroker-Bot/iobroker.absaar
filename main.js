...
class AbsaarAdapter extends utils.Adapter {
  ...
  async request(path, payload, json = true) {
    ...
    if (!data || data.code !== 200) {
      throw new Error(`API request ${path} failed: ${JSON.stringify(data)}`);
    }
    return data;
  }
  ...
  async fetchAllData() {
    await this.authenticate();
    const stationsData = await this.request(
      "/dn/power/station/listApp",
      { userId: this.userId },
      false,
    );
    if (!stationsData.rows) {
      throw new Error("No stations returned by Absaar API");
    }
    ...
    for (const station of stationsData.rows) {
      ...
      const collectorsData = await this.request(
        "/dn/power/collector/listByApp",
        { powerId },
        true,
      );
      for (const collector of collectorsData.rows || []) {
        const inverterId = String(collector.inverterId);
        const inverterData = await this.request(
          "/dn/power/inverterData/inverterDatalist",
          { powerId, inverterId },
          true,
        );
        stationInfo.collectors.push({
          ...
          collector_name:
            collector.collectorName || collector.inverterName || inverterId,
          collector_data: collector,
          inverter_data:
            inverterData.rows && inverterData.rows[0]
              ? inverterData.rows[0]
              : {},
          data: {
            ...collector,
  ...
  async post(path, payload, headers, json = true) {
    const base = String(
      this.config.baseUrl || "https://mini-ems.com:8081",
    ).replace(/\/$/, "");
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          ...
          try {
            ...
            reject(
              new Error(`Invalid JSON from ${path}: ${text.slice(0, 200)}`),
            );
          }
        });
      });
    ...
    for (const station of data.stations || []) {
      ...
      for (const [key, name, type, unit, role] of STATION_DEFINITIONS) {
        ...
        await this.setValue(
          `${stationBase}.${key}`,
          type,
          station.data ? station.data[key] : undefined,
        );
      }
      ...
      for (const collector of station.collectors || []) {
        ...
        await this.ensureChannel(
          base,
          collector.collector_name || collector.inverter_id,
        );
        for (const [key, name, type, unit, role] of COLLECTOR_DEFINITIONS) {
          ...
          await this.setValue(
            `${base}.${key}`,
            type,
            collector.data ? collector.data[key] : undefined,
          );
        }
  ...
  async setNumber(id, value) {
    ...
    if (Number.isFinite(num)) {
      await this.setStateAsync(id, num, true);
    }
  }
